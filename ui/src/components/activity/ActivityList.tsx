...
export function ActivityList({
  ...
  onLoadOlder,
  oldestBlockLoaded,
  emptyHint = "No activity in the last 10,000 blocks — Load older to look further back.",
}: ActivityListProps) {
  ...
  if (isLoading && events.length === 0) {
    return <p className="py-2 text-sm text-gray-400">Loading…</p>;
  }

  if (events.length === 0) {
    return <p className="py-2 text-sm text-gray-400">{emptyHint}</p>;
  }
